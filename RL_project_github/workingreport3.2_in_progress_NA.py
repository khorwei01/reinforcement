# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 12:28:46 2020
...
date_range = 3
data_opt1 = data_opt1.loc[data_opt1['date']<data_s['date'][30], :]
#PAYOFF COMPUTER
...
def state_control(numOfStrike, k, acc):
    if k == 0:
        return 1
    else:
        return k*2*(numOfStrike-1)*acc+1
...
if start_q_table is None:
    ...
    for k in range(date_range):
        ...
        for l in range(state_control(numOfStrike, k, acc=10)): #COVER ONLY RANGE OF OPTION PAIR TRADE
            q_table[k][l] = dict()  
...
epsilon = 0.9999
episode_rewards = []
HM_EPISODES = 10000 # better to have all combination (36*1)*(36*17)*(36*25)
EPS_DECAY = 0.999
put_call_parity = [0,1] # 0 equal to long, 1 equal to short
ran_date_range = 15 #BE CAREFUL OF TENOR RANGE, MAY NOT ABLE TO SEARCH DATA NOT IN RANGE
...
for i in range(HM_EPISODES):
    ...
    close = data_s['close'][np.random.randint(1, ran_date_range+1)]
    last = [close-spot, spot-close][train] #long position only
    payoffLast = []
    trade_hist = []
    for j in range(date_range):
        ...
        if j != 0:
            ...
            #close = last+spot
            tod_idx = np.random.randint(1, ran_date_range+1)
            close = data_s['close'][tod_idx]
        ...
        last = round(last)
        lower = -(numOfStrike-1)*(j+1)*5
        upper = (numOfStrike-1)*(j+1)*5
        ...
        if j == date_range-1:
            ...
            reward =  0
            last = round((last - payoffLast[j])*5)# - abs(premium[j]))
...
#Output qtable
#with open(f"q_table_3_2_sub.pickle", "wb") as f:
