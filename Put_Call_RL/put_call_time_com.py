# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 21:32:21 2020
...
def payoff(S=None, pick = None, choice = None, strike_list = None, call=None, put=None):
    if pick == choice-1 or pick == 2*choice-1:
        return 0,0
...
ex_date =  matureFinder(start_date, 1, 'M' , data_opt1) #select tenor
numOfStrike = 5
...
for i in range(HM_EPISODES):
    ...
    for j in range(date_range):
        ...
        #GET OPTION INFO 
        tod = data_opt1.loc[(data_opt1['date']==data_s['date'][j]) & (data_opt1['exdate']==ex_date),'forward_price'].values[0]
        strike_list = strikeListGen(data_s['date'][j], data_s['close'][j], data_opt1, int((numOfStrike-1)/2))       
        ...
        if np.random.random()  > epsilon: #
            ...
            action = action[0], action[1] + choice
        else:
        ...
        if j == date_range-1:
            ...
            reward = 0
            action = action[0], action[1] - choice # RECOVER ORIGINAL SIGN
...
#with open(f"qtable-put_chat_time.pickle", "wb") as f:
